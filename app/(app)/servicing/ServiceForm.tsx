...
const SERVICE_TYPES = ['routine_maintenance','oil_change','tire_replacement','brake_service','engine_repair','electrical_repair','bodywork','inspection','other']
...
export default function ServiceForm({ vehicles, branches, record, preselectedVehicleId }: Props) {
  ...
  const sel = (label: string, name: string, options: {v:string;l:string}[], def?: string, req = false) => (
    <div>
      <label className="block text-xs font-medium text-gray-400 mb-1">{label}{req ? ' *' : ''}</label>
      <select name={name} defaultValue={record ? (record as any)[name] : (def ?? '')} required={req}
  ...
  const inp = (label: string, name: string, type='text', req=false, extra?: any) => (
    <div>
      <label className="block text-xs font-medium text-gray-400 mb-1">{label}{req ? ' *' : ''}</label>
      <input type={type} name={name} required={req} defaultValue={record ? (record as any)[name] ?? '' : ''} {...extra}
        className="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-500" />
    </div>
  )
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        {sel('Vehicle *', 'vehicle_id', vehicles.map(v => ({ v: v._id, l: v.registration_number })), preselectedVehicleId, true)}
        {sel('Branch', 'branch_id', branches.map(b => ({ v: b._id, l: b.name })))}
        {sel('Service Type *', 'service_type', SERVICE_TYPES.map(t => ({ v: t, l: t.replace(/_/g,' ').replace(/\b\w/g, c=>c.toUpperCase()) })), undefined, true)}
        {sel('Status', 'status', ['pending','in_progress','completed','cancelled'].map(s => ({ v: s, l: s.replace(/_/g,' ') })), record?.status ?? 'pending')}
        {inp('Service Date *', 'service_date', 'date', true, { defaultValue: record?.service_date ? new Date(record.service_date).toISOString().slice(0,10) : '' })}
        {inp('Completion Date', 'completion_date', 'date', false, { defaultValue: record?.completion_date ? new Date(record.completion_date).toISOString().slice(0,10) : '' })}
        ...
        {inp('Workshop', 'workshop')}
      </div>
      <div>
        <label className="block text-xs font-medium text-gray-400 mb-1">Description *</label>
        <textarea name="description" required rows={3} defaultValue={record?.description ?? ''}
      ...
      <div>
        ...
        <textarea name="parts_replaced" rows={2} defaultValue={record?.parts_replaced ?? ''}
          className="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-500 resize-none" />
      </div>
